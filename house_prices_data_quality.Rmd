...
file_splits <- list(starts = c(1L, 
                               as.integer(round(nrow(output_table)/2)) + 1L), 
                    ends   = c(as.integer(round(nrow(output_table)/2)), 
                               nrow(output_table)), 
                    filenum = 1:2)

# Write once with Windows line endings
pwalk(file_splits, function(starts, ends, filenum) {
    fwrite(x = output_table[starts:ends], 
           file = paste0("output_table_pt", 
                         filenum, 
                         "_windows.txt"), 
           sep = "|", 
           eol = "\r\n", 
...
# Write again with normal line endings
pwalk(file_splits, function(starts, ends, filenum) {
    fwrite(x = output_table[starts:ends], 
           file = paste0("output_table_pt", 
                         filenum, 
